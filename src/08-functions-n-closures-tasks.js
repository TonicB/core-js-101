...
function getPolynom(...rest) {
  ...
  const k1 = args[2] ? args[2] : 0;
  const k2 = args[1] ? args[1] : 0;
  const k3 = args[0] ? args[0] : null;
  if (k3 === null) {
    return null;
  }
  return (x) => k1 * x * x + k2 * x + k3;
}
...
function logger(func, logFunc) {
  return (...args) => {
    const stringsFromArgs = args.map((x) => {
      if (Array.isArray(x)) {
        return JSON.stringify(x);
      }
      return x;
    });
    const nameOfFunc = func.name;
    logFunc(`${nameOfFunc}(${stringsFromArgs}) starts`);
    const res = func(...args);
    logFunc(`${nameOfFunc}(${stringsFromArgs}) ends`);
    return res;
  };
}
...
function partialUsingArguments(fn, ...args1) {
  const resFun = (...argsRes) => {
    if (args1.length >= fn.length) {
      return fn(...args1);
    }
    return fn(...args1, ...argsRes);
  };
  return resFun;
}
